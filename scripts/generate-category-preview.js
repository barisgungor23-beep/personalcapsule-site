...
const OUTPUT_DIR = path.join(ROOT, "outputs/generated/category");
...
function assetUrl(relativePath) {
  const version = SITE.assetVersion ? `?v=${encodeURIComponent(SITE.assetVersion)}` : "";
...
function readPublishedArticlesForCategory(categoryId) {
  const articlesDir = path.join(ROOT, "content/articles");
  return fs
    .readdirSync(articlesDir)
    .filter((name) => name.endsWith(".json"))
    .map((name) => readJson(`content/articles/${name}`))
    .filter((article) => article.status === "published" && article.category === categoryId)
    .sort((a, b) => {
...
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${escapeHtml(
    ...
    SITE.author
  )}"><meta name="theme-color" content="#fbf5ea"><link rel="icon" type="image/png" sizes="32x32" href="${SITE.siteUrl}/favicon-32x32.png"><link rel="apple-touch-icon" sizes="180x180" href="${SITE.siteUrl}/apple-touch-icon.png"><link rel="manifest" href="${SITE.siteUrl}/site.webmanifest"><link rel="canonical" href="${escapeAttr(
    canonical
  )}"><meta property="og:type" content="website"><meta property="og:site_name" content="${escapeAttr(
    SITE.siteName
    ...
    category.description
  )}"><meta name="twitter:image" content="${SITE.assets.ogImage}"><link rel="preload" href="../../fonts/hanken-grotesk-latin.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="../../fonts/fraunces-normal-latin.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="${assetUrl(
    "../../fonts.css"
  )}"><link rel="stylesheet" href="${assetUrl("../../styles.css")}"><script type="application/ld+json">${jsonLd(
    schema
  )}</script>
...
function renderCards(articles, category) {
  if (articles.length === 0) {
    return '<p class="category-intro">No articles are published in this category yet.</p>';
  }

  return `<div class="post-grid">${articles
    .map(
      (article) =>
        `<a class="post-card reveal" href="../${escapeAttr(article.slug)}"><span class="p-tag">${escapeHtml(
          category.name
...
function main() {
  ...
  for (const categoryId of categoryIds) {
    const category = readJson(`content/categories/${categoryId}.json`);
    const articles = readPublishedArticlesForCategory(category.id);
    const outputPath = path.join(OUTPUT_DIR, `${category.slug}.html`);
    fs.writeFileSync(outputPath, renderPage(category, articles));
